...
class Coarse_RoITransformer(BaseDetectorNew, RPNTestMixin):
    ...
    def __init__(self,
                 ...
                 bbox_head=None,
                 coarse_rbbox_roi_extractor=None,
                 coarse_rbbox_head=None,
        ...
        assert coarse_rbbox_roi_extractor is not None
        assert coarse_rbbox_head is not None

        assert rbbox_roi_extractor is not None
        assert rbbox_head is not None
        super(Coarse_RoITransformer, self).__init__()
        self.test_head = None
        ...
        if coarse_rbbox_head is not None:
            self.coarse_rbbox_roi_extractor = builder.build_roi_extractor(
                coarse_rbbox_roi_extractor)
            self.coarse_rbbox_head = builder.build_head(coarse_rbbox_head)
    ...
    def init_weights(self, pretrained=None):
        super(Coarse_RoITransformer, self).init_weights(pretrained)
        ...
        if self.with_coarserbbox:
            self.coarse_rbbox_roi_extractor.init_weights()
            self.coarse_rbbox_head.init_weights()
        if self.with_rbbox:
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            rbbox_targets = self.bbox_head.get_target(
                sampling_results, gt_masks, coarse_gt_labels, self.train_cfg.rcnn[0])
        ...
        #2021_09_23 liaoyujia for coarse
        if self.with_coarserbbox:
        ...
        if self.with_coarserbbox:
            ...
            # rrois[:, 3] = rrois[:, 3] * 1.2
            # rrois[:, 4] = rrois[:, 4] * 1.4
            rrois[:, 3] = rrois[:, 3] * self.coarse_rbbox_roi_extractor.w_enlarge
            rrois[:, 4] = rrois[:, 4] * self.coarse_rbbox_roi_extractor.h_enlarge
            rbbox_feats = self.coarse_rbbox_roi_extractor(x[:self.coarse_rbbox_roi_extractor.num_inputs],
                                                   rrois)
            if self.with_shared_head_rbbox:
                rbbox_feats = self.shared_head_rbbox(rbbox_feats)
            cls_score, rbbox_pred = self.coarse_rbbox_head(rbbox_feats)
            rbbox_targets = self.coarse_rbbox_head.get_target_rbbox(sampling_results, gt_obbs,
                                                        gt_labels, self.train_cfg.rcnn[1])
            loss_coarse_rbbox = self.coarse_rbbox_head.loss(cls_score, rbbox_pred, *rbbox_targets)
            # lw1 = self.train_cfg.stage_loss_weights[1]
            for name, value in loss_coarse_rbbox.items():
        ...
        with torch.no_grad():
            # import pdb
            # pdb.set_trace()
            coarse_rotated_proposal_list = self.bbox_head.refine_rbboxes(
                rrois, roi_labels, bbox_pred, pos_is_gts, img_meta
            )


        if self.with_rbbox:
            bbox_assigner = build_assigner(self.train_cfg.rcnn[2].assigner)
            bbox_sampler = build_sampler(
                self.train_cfg.rcnn[2].sampler, context=self)
            num_imgs = img.size(0)
            if gt_bboxes_ignore is None:
                gt_bboxes_ignore = [None for _ in range(num_imgs)]
            sampling_results = []
            for i in range(num_imgs):
                gt_obbs_best_roi = choose_best_Rroi_batch(gt_obbs[i])
                assign_result = bbox_assigner.assign(
                    coarse_rotated_proposal_list[i], gt_obbs_best_roi, gt_bboxes_ignore[i],
                    gt_labels[i])
                sampling_result = bbox_sampler.sample(
                    assign_result,
                    coarse_rotated_proposal_list[i],
                    torch.from_numpy(gt_obbs_best_roi).float().to(coarse_rotated_proposal_list[i].device),
                    gt_labels[i],
                    feats=[lvl_feat[i][None] for lvl_feat in x])
                sampling_results.append(sampling_result)
        ...
        if self.with_rbbox:
            ...
            rbbox_targets = self.rbbox_head.get_target_rbbox(sampling_results, gt_obbs,
                                                        gt_labels, self.train_cfg.rcnn[2])
            loss_rbbox = self.rbbox_head.loss(cls_score, rbbox_pred, *rbbox_targets)
            # lw2 = self.train_cfg.stage_loss_weights[2]
            for name, value in loss_rbbox.items():
                losses['s{}.{}'.format(2, name)] = (value)
                    # value * lw2 if 'loss' in name else value)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        rrois_enlarge[:, 3] = rrois_enlarge[:, 3] * self.coarse_rbbox_roi_extractor.w_enlarge
        rrois_enlarge[:, 4] = rrois_enlarge[:, 4] * self.coarse_rbbox_roi_extractor.h_enlarge
        rbbox_feats = self.coarse_rbbox_roi_extractor(
            x[:len(self.coarse_rbbox_roi_extractor.featmap_strides)], rrois_enlarge)
        if self.with_shared_head_rbbox:
            rbbox_feats = self.shared_head_rbbox(rbbox_feats)

        rcls_score, rbbox_pred = self.coarse_rbbox_head(rbbox_feats)

        #/ for coarse
        self.test_head = self.rbbox_head
        bbox_label = rcls_score.argmax(dim=1)
        rrois = self.rbbox_head.regress_by_class_rbbox(rrois, bbox_label, rbbox_pred,
                                                       img_meta[0])
        rrois_enlarge = copy.deepcopy(rrois)
